 ...
 * keys.
 *
 * @author Thomas Bolz. Modifed James A R Brown min/max values
 */
public class NumberSpinner extends HBox {
    ...
    private ObjectProperty<BigDecimal> minValueProperty = new SimpleObjectProperty<>();
    private boolean isMinValueSet = false;
    private ObjectProperty<BigDecimal> maxValueProperty = new SimpleObjectProperty<>();
    ...
    public void refreshButtons() {
        ...
        if (isMaxValueSet) {
            if (value.add(stepWidthProperty.get()).compareTo(maxValueProperty.get()) == 1) {
        ...
        if (isMinValueSet) {
            if (value.subtract(stepWidthProperty.get()).compareTo(minValueProperty.get()) == -1) {
    ...
    public void setMinValueProperty(BigDecimal bg) {
        this.minValueProperty.setValue(bg);
        this.setNumber(bg);
        decrementButton.setDisable(true);
        isMinValueSet = true;
    }

    public void setMaxValueProperty(BigDecimal bg) {
        this.maxValueProperty.setValue(bg);
        isMaxValueSet = true;
    }
